...
**Architecture:** 这是一次文案同步，不改变运行逻辑、数据流或依赖。通过精确替换已盘点的旧品牌文本，保留中文名称、仓库名、官网 URL 和功能描述。
...
- 仅更新用户可见的英文产品名。
- 保留中文产品名称及中文翻译文本。
- 保留 GitHub 仓库名 `SmartHyperlinkRecognition` 和官网 URL `smart-hyperlink-recognition.vercel.app`。
...
Do not replace `Smart Hyperlink Recognition` when it is part of a URL or repository identifier, and do not replace the Chinese-language parenthetical brand text unless it is an English-only user-facing product label.
...
rg -n "SmartHyperlinkRecognition|smart-hyperlink-recognition\\.vercel\\.app" README.md README.zh-CN.md extension website package.json
```
